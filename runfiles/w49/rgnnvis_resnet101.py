...
#                                             'yolact_resnet50_54_800000_base.pth'))
    ...
    backbone = IntermediateLayerGetter(resnet101(pretrained=False), yolact_bb_return_layers)
...
#    detector.load_state_dict_without_specific_classes(yolact_base_sd)
    ...
    saved_weights = torch.load(os.path.join(config['nn_weights_path'], 'vps063_v5_ep0120.pth.tar'))['net']
    ...
    if args.test is not None:
        ...
        for idx in checkpoint_ids:
            ...
            file_path = os.path.join(config['workspace_path'], checkpoint_name)
